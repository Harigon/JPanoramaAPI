    ...
    implements Runnable
{
    ...
    {
        if(pv == null || idata == null || pv.renderer.vdata == null)
            return;
        if(antialias && scaledImages == null)
        {
            ...
            int ai[][] = idata;
            double d = pv.renderer.hfov_max / ((double)pv.renderer.vwidth * chfov * max_oversampling);
            for(int i = 0; ai != null && (double)ai[0].length * d > 1.0D; i++)
            {
                ai = Utils.im_halfsize(ai);
            ...
            {
                abyte1 = Utils.im_halfsize(abyte1);
                scaledHSImages.addElement(abyte1);
            }

        }
        if(pv.renderer.dirty && (antialias || pv.renderer.hfov * (double)width < max_oversampling * chfov * (double)pv.renderer.vwidth))
        {
            int ai1[][] = idata;
            byte abyte0[][] = hsdata;
            if(antialias && scaledImages != null)
            {
                double d1 = pv.renderer.hfov / ((double)pv.renderer.vwidth * chfov * max_oversampling);
                int j = 0;
            ...
            case 0: // '\0'
                pv.renderer.dirty = !SetView(ai1, abyte0, false);
                return;

            case 1: // '\001'
                if(pv.panning || pv.renderer.lastframe > pv.renderer.frames)
                {
                    SetView(ai1, abyte0, false);
                    return;
                } else
                {
                    pv.renderer.dirty = !SetView(ai1, abyte0, true);
                    return;
                }
    ...
    boolean SetView(int ai[][], byte abyte0[][], boolean bilinear)
    {
        ...
        int k5 = ai[0].length;
        int l5 = ai.length;
        SetMatrix((pv.renderer.pitch * 2D * 3.1415926535897931D) / 360D, ((pv.renderer.yaw - cyaw) * 2D * 3.1415926535897931D) / 360D, 1);
        if(position == 1)
            ShiftCubeFace(1.5707963267948966D);
        else
        if(position == 2) 
            ShiftCubeFace(-1.5707963267948966D);
        double d1 = (double)pv.renderer.vwidth / (2D * Math.tan((pv.renderer.hfov * 3.1415926535897931D) / 180D / 2D));
        ...
        int w2 = pv.renderer.vwidth - 1 >> 1;
        int h2 = pv.renderer.vheight >> 1;
        int sw2 = (k5 << 7) + 64;
        int sh2 = (l5 << 7) + 64;
        int h1 = l5 - 1;
        int w1 = k5 - 1;
        ...
        if(bilinear)
        {
            ...
            int i9 = -h2 * k4 + j5 + 128;
            int k9 = -h2 * i4 + l4;
            for(int i10 = -h2 * j4 + i5; k < pv.renderer.vheight; i10 += j4)
            {
                int k7 = pv.renderer.vwidth * k;
                int i = 0;
                int k8 = k7;
                int k10 = -w2 * l3 + i9;
                int i11 = -w2 * j3 + k9;
                int k11 = -w2 * k3 + i10;
                while(i < pv.renderer.vwidth) 
                {
                    int l2;
                    if(pv.renderer.vdata[k8] == 0)
                        if((l2 = k10 + i * l3 >> 8) <= 0)
                        {
                            rename = false;
                        } else
                        {
                            ...
                            if((ys >>= 8) == l12 && xs >= 0 && xs < w1)
                            {
                                ...
                                {
                                    pv.renderer.hs_vdata[k8] = abyte1[xs];
                                    int i14 = ai2[xs++];
                                    int k13 = ai1[xs];
                                    int k14 = ai2[xs];
                                    pv.renderer.vdata[k8] = Utils.bil(i13, k13, i14, k14, dx, dy);
                                } else
                                {
                                    rename = false;
                                }
                            ...
                            if(xs >= 0 && ys >= 0 && ys < h1 && xs < w1)
                            {
                                ...
                                if(((j13 = ai1[xs]) & 0xff000000) != 0)
                                {
                                    ...
                                    int l13 = ai1[xs];
                                    int l14 = ai2[xs];
                                    pv.renderer.vdata[k8] = Utils.bil(j13, l13, j14, l14, dx, dy);
                                } else
                                {
                                    rename = false;
                                }
                            } else
                            {
                                ...
                                {
                                    pv.renderer.vdata[k8] = i12;
                                    pv.renderer.hs_vdata[k8] = abyte0[ys][xs];
                                } else
                                {
                                    rename = false;
                                }
        ...
        } else
        {
            ...
            int j9 = -h2 * k4 + j5 + 128;
            int l9 = -h2 * i4 + l4;
            for(int j10 = -h2 * j4 + i5; l < pv.renderer.vheight; j10 += j4)
            {
                int l7 = pv.renderer.vwidth * l;
                int j = 0;
                int l8 = l7;
                int l10 = -w2 * l3 + j9;
                int j11 = -w2 * j3 + l9;
                int l11 = -w2 * k3 + j10;
                while(j < pv.renderer.vwidth) 
                {
                    int i3;
                    if(pv.renderer.vdata[l8] == 0)
                        if((i3 = l10 + j * l3 >> 8) <= 0)
                        {
                            rename = false;
                        } else
    ...
    void SetMatrix(double d, double d1, int i)
    {
        ...
        ad1[2][2] = ad1[0][0];
        if(i == 1)
        	Utils.matrix_matrix_mult(ad, ad1, ad2);
        else
        	Utils.matrix_matrix_mult(ad1, ad, ad2);
        Utils.matrix_matrix_mult(ad2, f2, f1);
    }
    ...
    void SetDefMatrix()
    {
        ...
        ad1[2][1] = 0.0D;
        ad1[2][2] = 1.0D;
        Utils.matrix_matrix_mult(ad1, ad, f2);
    }
    ...
    void ShiftCubeFace(double d)
    {
        ...
        ad1[2][1] = 0.0D;
        ad1[2][2] = ad1[0][0];
        Utils.matrix_matrix_mult(f1, ad1, ad);
        for(int i = 0; i < 3; i++)
        {
            for(int j = 0; j < 3; j++)
                f1[i][j] = ad[i][j];
