		...
		do {
			...
			for( int k = 0; k < nTiles; k++ ){
				if( !tileData[k].loaded ) {
					...
					double distX = Math.abs( ptv.renderer.yaw - tileData[k].yaw );
					if( distX > 180 ) distX = 360 - distX;
					double distY = Math.abs( ptv.renderer.pitch - tileData[k].pitch );
...
//				System.out.println( "Loaded tile " + i + " pan=" + roi_pan[i] + " pano_pan=" + yaw);
	...
	protected boolean isTileVisible( int nTile ) {
		...
		double yawDist = Math.abs(ptv.renderer.yaw - tileData[nTile].yaw);
		if( yawDist > 180 ) yawDist = 360 - yawDist;
		double pitchDist = Math.abs( ptv.renderer.pitch - tileData[nTile].pitch );
		if( yawDist > (ptv.renderer.hfov + tileData[nTile].wDeg)/2 ) visible = false;
		if( visible ) {
			if( pitchDist > (Utils.math_fovy( ptv.renderer.hfov, ptv.renderer.vwidth, ptv.renderer.vheight) + tileData[nTile].hDeg)/2)
				visible = false;
		}
		return visible;
	}
